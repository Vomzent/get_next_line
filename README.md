...
`get_next_line` first checks if there are bytes inside it's buffer array. If there is not it will [read(2)](https://linux.die.net/man/2/read) to fill the buffer. Now that the we've ensured the buffer should be filled. We check if our buffer contains a newline or is empty. If it doesn't: we put everything in our buffer inside a [malloc(3)](https://linux.die.net/man/3/malloc)'d array and read from the buffer again looping and each time appending the string we have in buffer to the string we have from our previous [read(2)](https://linux.die.net/man/2/read) calls. Once we do find a newline or we stop recieving bytes from [read(2)](https://linux.die.net/man/2/read), We append the last bit of our line to the string we created. Move everything after the newline in the buffer to the front. And that's about it :)
